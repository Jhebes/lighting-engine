...
Why A*?
There's probably better lighting pattern algorithms somewhere, but I picked A* for a few reasons. Primarily A* doesn't need to preprocess the graph. This is pretty huge, as creating an optimal route table for every cell to every other cell could be pretty costly, and I didn't want to have the page take a few seconds to load in. Additionally, as soon as you place a wall, _every_single_table_ would have to update. That's really just not practical. Something like A* can run on the fly for each node and be reasonably speedy. It may be possible to run something like Dijkstra's for each node, but I haven't done the calculations to determine if this is faster than running A* to every other node on the graph. However, the choice of algorithm here I think is beyond the scope of this class, as the end product will be the same either way; it will only affect how many cells I can reasonably have in my grid. Right now, I find I really can't better that ~30x30 on my machine without lag. 


What's the technical achievement here?
The major interesting point here is the grid itself. The collection of both click and mousemove events form the basis, but taking those and actually using them to produce an interesting visual is pretty interesting. A lighting engine seems to be reasonably respectable as a technical achievement.   


What else is there?
I mean, if you're looking for other event listeners, the big ones other than the grid are the ones I use to format the page on load and resize, such as sizing the grid to the exact height of your window (pretty cool, huh?).
But if you really need something else, here's some buttons or something. The first shows bubbling, the second shows capturing. Output's in the console. Have fun.
